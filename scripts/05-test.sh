...
########################################################################
info "Calling ${BACKEND} 10x from frontend in eks-sim:"
for i in $(seq 1 10); do
  fe curl -s --max-time 5 "${BACKEND}/" || echo "(request failed)"
done | sed 's/^/    /'
echo
...
if fe curl -s --max-time 5 "${BACKEND}/" >/dev/null; then
  ok "frontend ALLOWED"
else
...
if bad curl -s --max-time 5 "${BACKEND}/" >/dev/null 2>&1; then
